...
执行中发现以下任一情况，调用 `scripts/sop_upgrade.py`：
1. 预计耗时超过 20 分钟
2. 涉及 ≥ 2 个系统
3. 需要多轮人工确认
4. 需要发布/重启等高影响操作
...
4. 创建：PLAN.md / DECISIONS.md / ARTIFACTS.md（从 templates/full/ 模板）
5. 更新 state.json：
   - `mode: "full"`
   - `status: "DISCUSSING"`
...
6. 创建快照（调用 EAM 底座 create_snapshot）
7. 同步 INDEX.md（调用 update_index.py）
